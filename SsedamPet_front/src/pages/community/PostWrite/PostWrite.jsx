...
const PostWrite = () => {
    ...
    const handleSubmit = async () => {
        if (!previewUrl || content.trim().length === 0 || !selectedPet) {
        ...
        formData.append("image", imageFile); // 실제 파일 전송
        formData.append("userId", 4); // 토큰에서 추출된 userId가 4였으므로 고정 또는 동적 할당
        ...
        <div css={s.rootContainer}>
            <main css={s.content}>
                ...
                <div css={s.imageUploadBox} onClick={handleImageClick}>
                    {previewUrl ? (
                        <img src={previewUrl} alt="미리보기" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px' }} />
                    ) : (
