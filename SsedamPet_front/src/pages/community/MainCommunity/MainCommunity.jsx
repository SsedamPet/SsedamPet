...
function MainCommunity() {
    ...
    const fetchPosts = async () => {
        try {
            ...
            const response = await axios.get(
                `${import.meta.env.VITE_API_BASE_URL}/api/community/posts?sortBy=${sortBy}`,
                {
                    headers: {
    ...
    const handleLike = async (postId) => {
        ...
        try {
            await axios.post(
                `${import.meta.env.VITE_API_BASE_URL}/api/community/post/${postId}/like`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("AccessToken")}`,
                ...
                {posts.map((post) => (
                    <div css={s.postCard} key={post.postId}>
                                ...
                                <img
                                    src={
                                        post.userProfileImgUrl
                                            ? post.userProfileImgUrl.startsWith(
                                                  "http",
                                              )
                                                ? post.userProfileImgUrl
                        ...
                        >
                            {post.postImgUrl && (
                                <img
                                    src={`${import.meta.env.VITE_API_BASE_URL}${post.postImgUrl.startsWith("/") ? "" : "/"}${post.postImgUrl}`}
                                    alt="게시글"
                                    css={s.postImgTag}
                                />
                            )}
